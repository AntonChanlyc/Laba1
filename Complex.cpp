...
Number* Complex::add(const Number& other) const {
    if (other.getType() != COMPLEX) throw std::runtime_error("Type mismatch: add Complex with non-Complex");
...
Number* Complex::sub(const Number& other) const {
    if (other.getType() != COMPLEX) throw std::runtime_error("Type mismatch: sub Complex with non-Complex");
    const Complex& c = dynamic_cast<const Complex&>(other);
    return new Complex(re_ - c.re_, im_ - c.im_);
}

Number* Complex::mul(const Number& other) const {
    if (other.getType() != COMPLEX) throw std::runtime_error("Type mismatch: mul Complex with non-Complex");
    const Complex& c = dynamic_cast<const Complex&>(other);
    return new Complex(re_ * c.re_ - im_ * c.im_, re_ * c.im_ + im_ * c.re_);
}

Number* Complex::div(const Number& other) const {
    if (other.getType() != COMPLEX) throw std::runtime_error("Type mismatch: div Complex with non-Complex");
    const Complex& c = dynamic_cast<const Complex&>(other);
    double d = c.re_*c.re_ + c.im_*c.im_;
    if (d == 0) throw std::runtime_error("Division by zero");
    return new Complex((re_*c.re_ + im_*c.im_) / d, (im_*c.re_ - re_*c.im_) / d);
}
